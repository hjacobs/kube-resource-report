...
sed -i "s/__version__ = .*/__version__ = '${VERSION}'/" ../report.py

sleep 10

(cd ../ && OAUTH2_ACCESS_TOKENS=read-only=mytok pipenv run ./report.py sample-report/output/ --application-registry $(minikube service application-registry --url) \
